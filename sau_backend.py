...
def delete_account():
    ...
    if not account_id or not account_id.isdigit():
        return jsonify(
            {"code": 400, "msg": "Invalid or missing account ID", "data": None}
        ), 400
    ...
    try:
        ...
        return jsonify(
            {"code": 500, "msg": f"delete failed: {str(e)}", "data": None}
        ), 500
...
def postVideo():
    ...
    try:
        match type:
            ...
            case _:
                return jsonify(
                    {"code": 400, "msg": f"不支持的平台类型: {type}", "data": None}
                ), 400
...
def upload_cookie():
    try:
        if "file" not in request.files:
            return jsonify(
                {"code": 400, "msg": "没有找到Cookie文件", "data": None}
            ), 400

        file = request.files["file"]
        if file.filename == "":
            return jsonify(
                {"code": 400, "msg": "Cookie文件名不能为空", "data": None}
            ), 400

        if not file.filename.endswith(".json"):
            return jsonify(
                {"code": 400, "msg": "Cookie文件必须是JSON格式", "data": None}
            ), 400
        ...
        if not account_id or not platform:
            return jsonify(
                {"code": 400, "msg": "缺少账号ID或平台信息", "data": None}
            ), 400
