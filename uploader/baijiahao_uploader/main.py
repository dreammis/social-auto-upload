...
class BaiJiaHaoVideo(object):
    ...
    async def set_schedule_time(self, page, publish_date):
        ...
        _publish_date_hour = f"{publish_date.hour}点"
        _publish_date_min = f"{publish_date.minute}分"
        ...
        await page.locator(
            f"div.rc-virtual-list  div.cheetah-select-item >> text={publish_date_day}"
        ).click()
        await page.wait_for_timeout(2000)
        ...
        for _ in range(3):
            try:
                ...
                await page.wait_for_selector(
                    "div.rc-virtual-list div.rc-virtual-list-holder-inner:visible",
                    timeout=5000,
                )
                break
            except Exception:
        ...
        await page.wait_for_timeout(2000)
        await (
            page.locator("div.rc-virtual-list:visible div.cheetah-select-item-option")
            .nth(random.randint(1, current_choice_hour - 3))
            .click()
        )
    ...
    async def set_schedule_publish(self, page, publish_date):
        while True:
            schedule_element = (
                page.locator("div.op-btn-outter-content >> text=定时发布")
                .locator("..")
                .locator("button")
            )
            try:
    ...
    async def ai2video(self, playwright: Playwright) -> None:
        ...
        news_items = (
            await page.locator(container_selector)
            .locator("div.py\-\[6px\].group.cursor-pointer")
            .all()
        )
                    ...
                    if await one_key_button.count() > 0:
                        ...
                        if is_disabled is None:
                            ...
                            try:
                                ...
                                tip_window = page.locator(
                                    "div:has-text('温馨提示') >> visible=true"
                                )
                                if await tip_window.count() > 0:
                                    print("[发现] 温馨提示窗口")

                                    # 定位并点击"知道了"按钮，设置较短的超时时间
                                    know_button = page.locator(
                                        "button:has-text('知道了')"
                                    )
