...
async def _save_xhs_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None=None, qrcode_callback=None) -> dict:
...
async def _is_xhs_login_completed(page: Page) -> bool:
    ...
    if not await login_box.count():
        return True
    try:
        if not await login_box.is_visible():
            return True
...
async def xiaohongshu_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool=LOCAL_CHROME_HEADLESS):
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        if not handle:
            result = _build_login_result(False, 'cookie_invalid', 'cookie文件不存在或已失效', account_file)
            return result if return_detail else False
        xiaohongshu_logger.info(_msg('🥹', 'cookie 失效了，准备打开浏览器重新登录'))
        result = await xiaohongshu_cookie_gen(account_file, qrcode_callback=qrcode_callback, headless=headless)
        return result if return_detail else result['success']
    result = _build_login_result(True, 'cookie_valid', 'cookie有效', account_file)
    return result if return_detail else True

async def xiaohongshu_cookie_gen(account_file, qrcode_callback=None, poll_interval: int=3, max_checks: int=100, headless: bool=LOCAL_CHROME_HEADLESS):
    if headless:
            ...
            for _ in range(max_checks):
                if await _is_xhs_login_completed(page):
                    ...
                    if await cookie_auth(account_file):
                        ...
                        result = _build_login_result(False, 'cookie_invalid', '小红书扫码流程结束，但 cookie 校验失败', account_file, qrcode_info, page.url)
                    return result
                await asyncio.sleep(poll_interval)
            result = _build_login_result(False, 'timeout', '等待小红书扫码登录超时', account_file, qrcode_info, page.url)
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError, RuntimeError) as exc:
...
class XiaoHongShuBaseUploader(BaseVideoUploader):
    ...
    def __init__(self, publish_date: datetime | int, account_file, publish_strategy: str=XIAOHONGSHU_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def validate_base_args(self):
        ...
        if self.publish_strategy not in {XIAOHONGSHU_PUBLISH_STRATEGY_IMMEDIATE, XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED}:
    ...
    async def set_location(self, page: Page, location: str='青岛市'):
        ...
        await page.wait_for_timeout(1000)
        flexible_xpath = f'//div[contains(@class, "d-popover") and contains(@class, "d-dropdown")]//div[contains(@class, "d-options-wrapper")]//div[contains(@class, "d-grid") and contains(@class, "d-options")]//div[contains(@class, "name") and text()="{location}"]'
        await page.wait_for_timeout(3000)
        try:
            location_option = await page.wait_for_selector(flexible_xpath, timeout=3000)
            if not location_option:
                location_option = await page.wait_for_selector(f'//div[contains(@class, "d-popover") and contains(@class, "d-dropdown")]//div[contains(@class, "d-options-wrapper")]//div[contains(@class, "d-grid") and contains(@class, "d-options")]/div[1]//div[contains(@class, "name") and text()="{location}"]', timeout=2000)
            ...
            try:
                all_options = await page.query_selector_all('//div[contains(@class, "d-popover") and contains(@class, "d-dropdown")]//div[contains(@class, "d-options-wrapper")]//div[contains(@class, "d-grid") and contains(@class, "d-options")]/div')
    ...
    async def fill_tags(self, page: Page) -> None:
        ...
        if len(self.tags) > max_tags:
            xiaohongshu_logger.warning(_msg('🏷️', f'标签数量 {len(self.tags)} 超过小红书上限 {max_tags}，只取前 {max_tags} 个: {self.tags[:max_tags]}'))
        ...
        for tag in self.tags:
            try:
                ...
                await page.locator('#creator-editor-topic-container').wait_for(state='visible', timeout=6000)
                ...
                xiaohongshu_logger.warning(_msg('🏷️', f'话题『{tag}』未出现候选，跳过该标签继续发布: {exc}'))
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    def __init__(self, title, file_path, tags, publish_date: datetime | int, account_file, thumbnail_path=None, desc: str | None=None, publish_strategy: str=XIAOHONGSHU_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def set_thumbnail(self, page: Page, thumbnail_path: str):
        ...
        cover_upload_dialog = cover_plugin_title.locator("xpath=ancestor::div[contains(@class, 'cover-plugin-preview')]").locator('div.cover > div.default:visible')
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        max_upload_retries = 120
        for _attempt in range(max_upload_retries):
            try:
            ...
            raise TimeoutError(f'小红书视频上传未在 {max_upload_retries * 2} 秒内完成')
        ...
        max_publish_retries = 30
        for _attempt in range(max_publish_retries):
            try:
                if self.publish_strategy == XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED:
                    await page.locator('button:has-text("定时发布")').click()
                else:
                    await page.locator('button:has-text("发布")').click()
                await page.wait_for_url('https://creator.xiaohongshu.com/publish/success?**', timeout=3000)
            ...
            raise TimeoutError(f'小红书视频发布未在 {max_publish_retries * 0.5} 秒内完成')
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        context = await browser.new_context(permissions=['geolocation'], storage_state=self.account_file)
        context = await set_init_script(context)
        try:
            page = await context.new_page()
...
class XiaoHongShuNote(XiaoHongShuBaseUploader):
    ...
    def __init__(self, image_paths, note, tags, publish_date: datetime | int, account_file, title: str | None=None, desc: str | None=None, publish_strategy: str=XIAOHONGSHU_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
        super().__init__(publish_date=publish_date, account_file=account_file, publish_strategy=publish_strategy, debug=debug, headless=headless)
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        max_note_upload_retries = 60
        for _attempt in range(max_note_upload_retries):
            try:
            ...
            raise TimeoutError(f'小红书图文上传未在 {max_note_upload_retries} 秒内完成')
        xiaohongshu_logger.info(_msg('✍️', '小人开始填标题、描述和话题'))
        await self.fill_meta(page)
        await self.check_original_declaration(page)
        if self.publish_strategy == XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED and self.publish_date != 0:
        ...
        for _attempt in range(max_publish_retries):
            try:
                if self.publish_strategy == XIAOHONGSHU_PUBLISH_STRATEGY_SCHEDULED:
                    ...
                    await page.locator('button:has-text("发布")').click()
                await page.wait_for_url(XHS_PUBLISH_SUCCESS_URL_PATTERN, timeout=3000)
            ...
            raise TimeoutError(f'小红书图文发布未在 {max_publish_retries * 0.5} 秒内完成')
