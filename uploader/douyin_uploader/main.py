...
async def _extract_douyin_qrcode_src(page: Page) -> str:
    ...
    qrcode_img = scan_login_tab.locator('..').locator('xpath=following-sibling::div[1]').locator('img[aria-label="二维码"]').first
...
async def _is_douyin_login_completed(page: Page) -> bool:
    if not page.url.startswith('https://creator.douyin.com/creator-micro/home'):
        return False
    login_markers = [page.get_by_text('扫码登录', exact=True).first, page.get_by_text('手机号登录', exact=True).first, page.get_by_text('二维码失效', exact=True).first, page.get_by_role('img', name='二维码').first]
...
async def douyin_cookie_gen(account_file, qrcode_callback=None, poll_interval: int=3, max_checks: int=100, headless: bool=LOCAL_CHROME_HEADLESS):
    async with async_playwright() as playwright:
        ...
        try:
            ...
            result = await _wait_for_douyin_login(page, account_file, qrcode_info, qrcode_callback=qrcode_callback, poll_interval=poll_interval, max_checks=max_checks)
            if result['success']:
                await asyncio.sleep(2)
                await context.storage_state(path=account_file)
                if not await cookie_auth(account_file):
                    result = _build_login_result(False, 'cookie_invalid', '抖音扫码流程结束，但 cookie 校验失败', account_file, qrcode_info, page.url)
...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    def __init__(self, publish_date: datetime | int, account_file, publish_strategy: str=DOUYIN_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def _scrape_post_publish_info(self, page: Page) -> dict:
        """Scrape video URL and publish status from the manage page after publishing."""
        result: dict = {'video_url': '', 'publish_status': '', 'verified': False}
        try:
            await asyncio.sleep(3)
            current_url = page.url
            if '/content/manage' not in current_url:
                douyin_logger.debug(_msg('🧍', f'当前不在管理页: {current_url}，跳过抓取'))
                return result
            status_element = page.locator('[class*="status"]').first
            if await status_element.count():
                status_text = (await status_element.inner_text()).strip()
                result['publish_status'] = status_text
                douyin_logger.info(_msg('📋', f'发布状态: {status_text}'))
            link_element = page.locator('a[href*="video"]').first
            if await link_element.count():
                ...
                if href:
                    ...
                    douyin_logger.info(_msg('🔗', f'视频链接: {href}'))
            cards = page.locator('[class*="card"], [class*="item"], tr[class*="row"]')
            card_count = await cards.count()
            if card_count > 0:
                result['verified'] = True
                douyin_logger.info(_msg('✅', f'内容管理页检测到 {card_count} 个内容卡片，发布已确认'))
            else:
                result['verified'] = False
                douyin_logger.warning(_msg('⚠️', '内容管理页未检测到内容卡片，视频可能未真正发布'))
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError) as exc:
            douyin_logger.warning(_msg('😵', f'抓取发布信息失败: {exc}'))
        return result
    ...
    async def fill_title_and_description(self, page: Page, title: str, description: str, tags: list[str] | None=None):
        description_section = page.get_by_text('作品描述', exact=True).locator('xpath=ancestor::div[2]').locator('xpath=following-sibling::div[1]')
...
class DouYinVideo(DouYinBaseUploader):
    ...
    def __init__(self, title, file_path, tags, publish_date: datetime | int, account_file, thumbnail_landscape_path=None, productLink='', productTitle='', thumbnail_portrait_path=None, desc: str | None=None, publish_strategy: str=DOUYIN_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def upload(self, playwright: Playwright) -> dict:
        ...
        while True:
            try:
                await page.wait_for_url('https://creator.douyin.com/creator-micro/content/publish?enter_from=publish_page', timeout=3000)
                douyin_logger.info(_msg('🥳', '已经进入 version_1 发布页面'))
                break
            except (patchright.async_api.Error, OSError, asyncio.TimeoutError):
                try:
                    await page.wait_for_url('https://creator.douyin.com/creator-micro/content/post/video?enter_from=publish_page', timeout=3000)
        ...
        while True:
            try:
                ...
                if await publish_button.count():
                    ...
                    if is_disabled:
                        douyin_logger.warning(_msg('⚠️', '发布按钮当前不可点击，等待中...'))
                        await asyncio.sleep(1)
                        publish_attempts += 1
                        if publish_attempts >= max_publish_attempts:
                            raise RuntimeError('发布按钮持续不可点击，可能页面状态异常')
                        continue
                    await publish_button.click()
                    douyin_logger.info(_msg('🖱️', '已点击发布按钮，等待页面跳转...'))
                ...
                self.upload_result = await self._scrape_post_publish_info(page)
                break
            except RuntimeError:
                raise
            except (patchright.async_api.Error, OSError, asyncio.TimeoutError):
                publish_attempts += 1
                if publish_attempts >= max_publish_attempts:
                    douyin_logger.error(_msg('😵', f'已尝试发布 {publish_attempts} 次仍未成功，放弃发布'))
                    raise RuntimeError(f'发布失败：已尝试 {publish_attempts} 次，页面未跳转到内容管理页')
                await self.handle_auto_video_cover(page)
                douyin_logger.info(_msg('🏃', f'小人正在冲刺发布视频 (第{publish_attempts}次尝试)'))
                if self.debug:
    ...
    async def douyin_upload_video(self) -> dict:
        async with async_playwright() as playwright:
            return await self.upload(playwright)

    async def main(self) -> dict:
        return await self.douyin_upload_video()

class DouYinNote(DouYinBaseUploader):

    def __init__(self, image_paths, note, tags, publish_date: datetime | int, account_file, title: str | None=None, publish_strategy: str=DOUYIN_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
        super().__init__(publish_date=publish_date, account_file=account_file, publish_strategy=publish_strategy, debug=debug, headless=headless)
        ...
        while True:
            try:
                ...
                if await publish_button.count():
                    ...
                    await publish_button.click()
                    douyin_logger.info(_msg('🖱️', '已点击发布按钮，等待页面跳转...'))
                await page.wait_for_url('**/creator-micro/content/manage?enter_from=publish**', timeout=3000)
                ...
                if publish_attempts >= max_publish_attempts:
                    douyin_logger.error(_msg('😵', f'已尝试发布 {publish_attempts} 次仍未成功，放弃发布'))
                    raise RuntimeError(f'发布失败：已尝试 {publish_attempts} 次，页面未跳转到内容管理页')
                douyin_logger.info(_msg('🏃', f'小人正在冲刺发布图文 (第{publish_attempts}次尝试)'))
                await asyncio.sleep(0.5)
    ...
    async def douyin_upload_note(self) -> dict:
        async with async_playwright() as playwright:
            return await self.upload(playwright)