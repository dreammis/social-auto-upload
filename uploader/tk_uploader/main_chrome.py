...
class TiktokVideo(object):
    def __init__(
        self, title, file_path, tags, publish_date, account_file, thumbnail_path=None
    ):
        self.title = title
    ...
    async def set_schedule_time(self, page, publish_date):
        ...
        if await self.locator_base.locator(
            ...
            await self.locator_base.locator(
                "div.TUXButton-content >> text=Allow"
            ).click()
        ...
        if n_calendar_month != schedule_month:
            if n_calendar_month < schedule_month:
                arrow = self.locator_base.locator(
    ...
    async def handle_upload_error(self, page):
        ...
        select_file_button = self.locator_base.locator(
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        await page.wait_for_url(
            "https://www.tiktok.com/tiktokstudio/upload", timeout=10000
        )
    ...
    async def upload_thumbnails(self, page):
        ...
        await (
            self.locator_base.locator("div.cover-edit-panel:not(.hide-panel)")
    ...
    async def click_publish(self, page):
        _success_flag_div = "div.common-modal-confirm-modal"
        while True:
            try:
                ...
                await page.wait_for_url(
                    "https://www.tiktok.com/tiktokstudio/content", timeout=3000
                )
    ...
    async def get_last_video_id(self, page):
        ...
        video_list_locator = self.locator_base.locator(
        ...
        if await video_list_locator.count():
            ...
            video_id = (
                re.search(r"video/(\d+)", first_video_obj).group(1)
                if first_video_obj
                else None
            )
            return video_id
            ...
            try:
                ...
                if (
                    ...
                    if await self.locator_base.locator(
                        ...
                        tiktok_logger.info(
                            "  [-] found some error while uploading now retry..."
                        )
