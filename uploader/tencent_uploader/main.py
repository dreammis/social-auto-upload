...
TENCENT_UPLOAD_NOTE_URL = 'https://channels.weixin.qq.com/platform/post/create?type=image'
...
TENCENT_NOTE_MAX_TAGS = 10
TENCENT_NOTE_MAX_TITLE_LEN = 30
_UPLOAD_PROGRESS_SELECTORS = ('div.weui-desktop-loading', 'div[class*="uploading"]', 'span:has-text("上传中")', 'div:has-text("上传中"):has-text("%")')
...
def _get_qrcode_utils():
    ...
    return {'build_login_qrcode_path': build_login_qrcode_path, 'decode_qrcode_from_path': decode_qrcode_from_path, 'print_terminal_qrcode': print_terminal_qrcode, 'remove_qrcode_file': remove_qrcode_file, 'save_data_url_image': save_data_url_image}
...
async def cookie_auth(account_file):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            login_markers = [page.get_by_text('扫码登录', exact=True).first, page.get_by_text('发表视频', exact=True).first, page.get_by_role('button', name='发表').first]
...
async def _extract_tencent_qrcode_src(page: Page) -> str:
    if hasattr(page, 'frame_locator'):
        try:
            ...
            pass
    selector_candidates = ['div.login-qrcode-wrap img.qrcode', 'div.qrcode-wrap img.qrcode', 'img.qrcode', 'img[src^="data:image/"]']
...
async def _save_tencent_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None=None, qrcode_callback=None) -> dict:
    ...
    qrcode_path = qrcode_utils['save_data_url_image'](qrcode_src, qrcode_utils['build_login_qrcode_path'](account_file, suffix='tencent_login_qrcode'))
    ...
    if qrcode_content:
        ...
        tencent_logger.warning(_msg('😵', f'没能从二维码图片里解析出可打印内容，所以这次没法在终端重绘二维码；请直接打开 {qrcode_path} 扫码'))
...
async def _is_tencent_login_completed(page: Page) -> bool:
    publish_markers = [page.locator('div:has-text("发表视频")').first, page.locator('button:has-text("发表")').first, page.locator('button:has-text("保存草稿")').first]
    ...
    if 'channels.weixin.qq.com' in page.url:
        login_markers = [page.locator('div.login-qrcode-wrap').first, page.locator('div.qrcode-wrap').first, page.locator('img.qrcode').first, page.locator('span:has-text("微信扫码登录 视频号助手")').first]
...
async def _is_tencent_qrcode_expired(page: Page) -> bool:
    tip_selectors = ['div.mask.show p.refresh-tip:has-text("二维码已过期，点击刷新")', 'div.mask.show p.refresh-tip:has-text("网络不可用，点击刷新")', 'p.refresh-tip:has-text("二维码已过期，点击刷新")', 'p.refresh-tip:has-text("网络不可用，点击刷新")']
...
async def _is_tencent_qrcode_scanned(page: Page) -> bool:
    scanned_tips = ['div.qr-tip div:has-text("已扫码")', 'div.qr-tip div:has-text("需在手机上进行确认")']
...
async def _refresh_tencent_qrcode(page: Page) -> None:
    visible_refresh_selectors = ['div.login-qrcode-wrap div.mask.show div.refresh-wrap', 'div.login-qrcode-wrap div.mask.show .refresh-wrap']
...
async def _wait_for_tencent_login(page: Page, account_file: str, qrcode_info: dict, qrcode_callback=None, poll_interval: int=3, max_checks: int=100) -> dict:
    ...
    for _ in range(max_checks):
        ...
        if await _is_tencent_qrcode_expired(page):
            ...
            qrcode_info = await _save_tencent_qrcode(page, account_file, previous_qrcode_path=qrcode_path, qrcode_callback=qrcode_callback)
            qrcode_path = Path(qrcode_info['image_path'])
        await asyncio.sleep(poll_interval)
    return _build_login_result(False, 'timeout', '等待视频号扫码登录超时', account_file, qrcode_info, page.url)

async def tencent_cookie_gen(account_file, qrcode_callback=None, poll_interval: int=3, max_checks: int=100, headless: bool=LOCAL_CHROME_HEADLESS):
    account_file = _resolve_account_file(account_file)
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            result = await _wait_for_tencent_login(page, account_file, qrcode_info, qrcode_callback=qrcode_callback, poll_interval=poll_interval, max_checks=max_checks)
            if result['success']:
                await asyncio.sleep(2)
                await context.storage_state(path=account_file)
                if not await cookie_auth(account_file):
                    result = _build_login_result(False, 'cookie_invalid', '视频号扫码流程结束，但 cookie 校验失败', account_file, qrcode_info, page.url)
            return result
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError, RuntimeError) as exc:
...
async def tencent_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool=LOCAL_CHROME_HEADLESS):
    account_file = _resolve_account_file(account_file)
...
async def weixin_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool=LOCAL_CHROME_HEADLESS):
    return await tencent_setup(account_file, handle=handle, return_detail=return_detail, qrcode_callback=qrcode_callback, headless=headless)

class TencentBaseUploader(BaseVideoUploader):

    def __init__(self, publish_date: datetime | int, account_file, publish_strategy: str=TENCENT_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def apply_original_statement(self, page: Page) -> None:
        ...
        if label_locator:
            ...
            original_set = True
        declaration_entry = page.locator('div.label span:has-text("声明原创"), div:has-text("声明原创"):has(input.ant-checkbox-input), div:has-text("原创声明"):has(input.ant-checkbox-input)').first
        if await declaration_entry.count():
            ...
            if await original_checkbox.count() and (not await original_checkbox.is_disabled()):
                ...
                checked_locator = page.locator('div.declare-original-dialog label.ant-checkbox-wrapper.ant-checkbox-wrapper-checked:visible')
            ...
            if await original_type_form.count():
                ...
                option = None
                if category:
                    option = page.locator(f'ul.weui-desktop-dropdown__list li.weui-desktop-dropdown__list-ele:has-text("{category}")').first
                    if not await option.count():
                        option = None
                if option is None:
                    option = page.locator('ul.weui-desktop-dropdown__list li.weui-desktop-dropdown__list-ele:visible').first
    ...
    async def wait_for_upload_complete(self, page: Page) -> None:
        ...
        for _attempt in range(max_retries):
            ...
            raise TimeoutError(f'视频号视频上传未在 {max_retries * 2} 秒内完成')
    ...
    async def submit_publish(self, page: Page) -> None:
        ...
        for _attempt in range(max_retries):
            try:
                ...
                if getattr(self, 'is_draft', False):
                    if 'post/list' in current_url or 'draft' in current_url:
                        tencent_logger.success(_msg('🥳', '视频草稿保存成功'))
                        break
                elif TENCENT_MANAGE_URL in current_url:
                    tencent_logger.success(_msg('🥳', '视频发布成功'))
                    break
            ...
            raise TimeoutError(f'视频号发布未在 {max_retries * 0.5} 秒内完成')

class TencentVideo(TencentBaseUploader):

    def __init__(self, title, file_path, tags, publish_date: datetime | int, account_file, category=None, is_draft=False, desc: str | None=None, thumbnail_path: str | None=None, thumbnail_landscape_path: str | None=None, thumbnail_portrait_path: str | None=None, short_title: str | None=None, publish_strategy: str=TENCENT_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS):
    ...
    async def confirm_thumbnail_crop(self, page: Page) -> None:
        ...
        try:
            ...
            crop_confirm_button = crop_dialog.locator('div.weui-desktop-dialog__ft button.weui-desktop-btn_primary:has-text("确定")').first
    ...
    async def upload_thumbnail_in_dialog(self, page: Page, cover_dialog, thumbnail_path: str) -> None:
        ...
        confirm_button = cover_dialog.locator('div.weui-desktop-dialog__ft button.weui-desktop-btn_primary:has-text("确认")').first
        await confirm_button.wait_for(state='visible', timeout=10000)
        await confirm_button.click()

    async def set_single_thumbnail(self, page: Page, thumbnail_path: str, selectors: list[str], dialog_titles: list[str], label: str) -> None:
    ...
    async def set_thumbnail(self, page: Page) -> None:
        ...
        landscape_selectors = ['div.horizontal-cover-wrap:has-text("4:3")', 'div[class*="cover-wrap"]:has-text("4:3"):has-text("动态")', 'div:has-text("视频号动态"):has-text("4:3")', 'div:has-text("横版封面"):has-text("4:3")']
        portrait_selectors = ['div.vertical-cover-wrap:has-text("个人主页卡片"):has-text("3:4")', 'div.vertical-cover-wrap:has-text("3:4")', 'div.vertical-cover-wrap:has-text("个人主页卡片")']
        if self.thumbnail_landscape_path:
            await self.set_single_thumbnail(page, self.thumbnail_landscape_path, landscape_selectors, ['编辑视频号动态封面', '编辑动态封面', '编辑封面'], '4:3 横版')
        if self.thumbnail_portrait_path:
            await self.set_single_thumbnail(page, self.thumbnail_portrait_path, portrait_selectors, ['编辑个人主页卡片', '编辑封面'], '3:4 竖版')
...
class TencentNote(TencentBaseUploader):
    ...
    def __init__(self, image_paths, note, tags, publish_date: datetime | int, account_file, title: str | None=None, publish_strategy: str=TENCENT_PUBLISH_STRATEGY_IMMEDIATE, debug: bool=DEBUG_MODE, headless: bool=LOCAL_CHROME_HEADLESS, is_draft: bool=False):
        super().__init__(publish_date=publish_date, account_file=account_file, publish_strategy=publish_strategy, debug=debug, headless=headless)
    ...
    async def validate_upload_args(self):
        ...
        max_title_len = TENCENT_NOTE_MAX_TITLE_LEN
        if len(self.title) > max_title_len:
            tencent_logger.warning(_msg('✂️', f'title 长度 {len(self.title)} 超过视频号图文建议上限 {max_title_len}，已截断'))
    ...
    async def _is_in_note_mode(self, page: Page) -> bool:
        ...
        只看「图文」提示型元素的可见性，不再要求「视频」提示同时为不可见——SPA 模式下两个
        tab 会在动画过渡中同时可见，此时只能采纳正向信号。括号中的``是图文的乐观检测``用
        只要看到「发表图文 / 图片上传 / 请上传图片」中任意一个可视元素即返回 True。
        """
        candidates = [page.locator('div:has-text("发表图文")').first, page.locator('div:has-text("图片上传")').first, page.locator('div:has-text("请上传图片")').first, page.locator('button:has-text("发表图文")').first]
    ...
    async def switch_to_note_mode(self, page: Page) -> None:
        ...
        优先顺序：
        1. 直接命中 URL 加 ``?type=image``（服务端可能直接渲染图文编辑）；
        2. 顶部 tab / 列表项 / 按钮文本为「图文」、「发表图文」、「图片」；
        3. 退回到左侧导航。
        全部失效时仅警告、不抛 RuntimeError，给上层一个明确「已经按图文走，后续失败请人工介入」。"""
        tencent_logger.info(_msg('🔀', '小人正在尝试切换到视频号图文发布模式'))
        try:
            target_url = TENCENT_UPLOAD_NOTE_URL
            if page.url != target_url:
                await page.goto(target_url)
                await page.wait_for_load_state('domcontentloaded')
                await asyncio.sleep(1)
            if await self._is_in_note_mode(page):
                tencent_logger.info(_msg('🥳', '通过 type=image 直跳成功进入图文模式'))
                return
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError) as exc:
            tencent_logger.debug(_msg('🧍', f'URL 直跳失败，继续尝试页面内的切换入口: {exc}'))
        text_strategies = [page.get_by_text('发表图文', exact=False).first, page.get_by_text('图文', exact=True).first, page.get_by_role('tab', name='图文').first, page.locator('div[role="tab"]:has-text("图文")').first, page.locator('li[role="tab"]:has-text("图文")').first, page.locator('button:has-text("图文")').first]
        for locator in text_strategies:
            try:
                if await locator.count() and await locator.is_visible():
                    await locator.click()
                    await asyncio.sleep(1)
                    if await self._is_in_note_mode(page):
                        tencent_logger.info(_msg('🥳', '已切换到视频号图文模式'))
                        return
            except (patchright.async_api.Error, OSError, asyncio.TimeoutError):
                continue
        tencent_logger.warning(_msg('😵', '未能明确找到「图文」入口。可能是当前账号无图文权限或 UI 已改版，请参考 debug 截图人工确认。'))

    async def upload_note_images(self, page: Page) -> None:
        """上传图片到图文模式编辑器，等待上传完成。

        安全策略：
        - 一次性传入的图片数量超过 ``TENCENT_NOTE_MAX_IMAGES`` 时按限额截断并打 warning。
        - 通过 file input + 编辑器预览/loading 状态判断上传完成。
        - 超过进度等待阈值抛 ``TimeoutError``，与 ``fill_title_and_tags`` 的容错策略一致。
        """
        tencent_logger.info(_msg('📤', f'小人开始上传图文图片，共 {len(self.image_paths)} 张'))
        if len(self.image_paths) > TENCENT_NOTE_MAX_IMAGES:
            tencent_logger.warning(_msg('⚠️', f'图片数量 {len(self.image_paths)} 超过视频号图文安全上限 {TENCENT_NOTE_MAX_IMAGES}，仅上传前 {TENCENT_NOTE_MAX_IMAGES} 张'))
            self.image_paths = self.image_paths[:TENCENT_NOTE_MAX_IMAGES]
        file_input = page.locator('input[type="file"][accept*="image"]').first
        if not await file_input.count():
            file_input = page.locator('div[class*="upload"] input[type="file"]').first
        if not await file_input.count():
            raise RuntimeError("未找到视频号图文图片上传输入框（input[type=file][accept*='image']）")
        if not await self._is_in_note_mode(page):
            tencent_logger.warning(_msg('😵', '准备上传图片但页面仍非图文模式，后续输入可能冲突——请检查 UI 是否改版'))
        ...
        for attempt in range(max_retries):
            try:
                uploading_counts = 0
                for sel in _UPLOAD_PROGRESS_SELECTORS:
                    uploading_counts += await page.locator(sel).count()
                publish_btn = page.locator('div.form-btns button:has-text("发表")').first
                publish_disabled = False
                if await publish_btn.count():
                    cls = await publish_btn.get_attribute('class') or ''
                    publish_disabled = 'weui-desktop-btn_disabled' in cls
                if uploading_counts == 0 and (not publish_disabled):
                    tencent_logger.success(_msg('🥳', '图文素材已全部传完'))
                    return
            except (patchright.async_api.Error, OSError, asyncio.TimeoutError) as exc:
                tencent_logger.debug(_msg('🧍', f'检查图片上传状态时遇到小波折: {exc}'))
            await asyncio.sleep(1)
            if attempt % 10 == 0 and attempt > 0:
                tencent_logger.info(_msg('🏃', f'小人继续等待图文上传完成，已尝试 {attempt} 次'))
        raise TimeoutError(f'视频号图文图片上传未在 {max_retries} 秒内完成')

    async def fill_note_title_and_tags(self, page: Page) -> None:
        """填写标题、正文和话题。

        视频号图文与视频共用同一个 ``div.input-editor`` 富文本编辑器（contenteditable），
        写入顺序为「标题 → Enter → 正文 → Enter → 话题 #tag + 空格」。话题超过
        ``TENCENT_NOTE_MAX_TAGS`` 时截断并 warning，避免平台风控拒发。
        """
        tencent_logger.info(_msg('✍️', '小人开始填写图文标题、正文和话题'))
        editor = page.locator('div.input-editor').first
        try:
            await editor.wait_for(state='visible', timeout=10000)
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError) as exc:
            raise RuntimeError(f'未在图文页面找到标题/正文编辑器（div.input-editor）: {exc}')
        await editor.click()
        try:
            await page.evaluate("(el) => { const e = document.querySelector(el) || document.activeElement; if (e) { e.focus(); document.execCommand && document.execCommand('selectAll'); } }", 'div.input-editor')
        except (patchright.async_api.Error, OSError, asyncio.TimeoutError):
        ...
        if self.note:
            ...
            await page.keyboard.press('Enter')
        if self.tags:
            if len(self.tags) > TENCENT_NOTE_MAX_TAGS:
                tencent_logger.warning(_msg('🏷️', f'话题数量 {len(self.tags)} 超过视频号建议上限 {TENCENT_NOTE_MAX_TAGS}，仅取前 {TENCENT_NOTE_MAX_TAGS} 个'))
                self.tags = self.tags[:TENCENT_NOTE_MAX_TAGS]
            for tag in self.tags:
                await page.keyboard.type(f'#{tag}')
                await page.keyboard.press('Space')
                await asyncio.sleep(0.3)
        tencent_logger.info(_msg('🏷️', f'已写入 {len(self.tags or [])} 个话题'))

    async def fill_note_body(self, page: Page) -> None:
        """图文模式下，``div.input-editor`` 同时承担「标题 + 正文 + 话题」。

        已在上一步 ``fill_note_title_and_tags`` 顺序写入，这里保留空实现，避免重复键盘事件导致富文本错乱。
        """
        return None
