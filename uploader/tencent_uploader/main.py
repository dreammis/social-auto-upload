        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                await page.wait_for_url(
                    "https://channels.weixin.qq.com/platform", timeout=120000
                )  # 2分钟超时
            ...
            if account_info:
                ...
                tencent_logger.success(
                    f'[+]成功获取账号 {account_info["nickname"]} 的cookie'
                )
...
async def weixin_setup(account_file, handle=False):
    ...
    try:
        ...
        if handle:
            tencent_logger.info(
                "[+] cookie文件不存在或已失效，即将自动打开浏览器，请扫码登录"
            )
...
class TencentVideo(object):
    ...
    async def upload_single(self, account_file: str) -> None:
        ...
        async with async_playwright() as playwright:
            try:
                ...
                cookie_file = (
                    Path(account_file).parent
                    / f"{account_name}_{int(time.time())}.json"
                )

                # 访问发布页面
                await page.goto("https://channels.weixin.qq.com/platform/post/create")
                tencent_logger.info(
                    f"[+][{account_name}] 开始上传视频-------{self.title}"
                )
                tencent_logger.info(
                    f'   [-][{account_name}] 当前视频数: {account_info["video_count"]}'
                )
                await page.wait_for_url(
                ...
                tencent_logger.error(
                    f"账号 {Path(account_file).stem} 上传失败: {str(e)}"
                )
            finally:
    ...
    async def get_account_info(page) -> dict:
        ...
        try:
            ...
            info = {
                "nickname": nickname,
                "id": await page.locator("#finder-uid-copy").get_attribute(
                    "data-clipboard-text"
                )
                or "",
                "video_count": await page.locator(
                    ".finder-content-info .finder-info-num"
                ).first.inner_text()
                or "0",
                "follower_count": await page.locator(
                    ".second-info .finder-info-num"
                ).inner_text()
                or "0",
            }
