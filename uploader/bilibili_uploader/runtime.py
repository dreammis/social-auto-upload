...
def _select_release_asset(assets: list[dict]) -> dict:
    ...
    preferred_patterns = {'windows-x86_64': ('x86_64-windows.zip',), 'linux-x86_64': ('x86_64-linux.tar.xz',), 'linux-aarch64': ('aarch64-linux.tar.xz',), 'linux-arm': ('arm-linux.tar.xz',), 'macos-x86_64': ('x86_64-macos.tar.xz',), 'macos-aarch64': ('aarch64-macos.tar.xz',)}
...
def fetch_latest_release() -> dict:
    ...
    return {'tag_name': payload.get('tag_name', ''), 'asset_name': selected_asset['asset_name'], 'asset_url': selected_asset['asset_url']}
