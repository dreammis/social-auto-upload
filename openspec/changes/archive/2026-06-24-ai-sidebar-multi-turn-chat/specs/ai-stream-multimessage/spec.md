# Capability: ai-stream-multimessage

## Purpose

Augment the existing `/api/ai/generate/stream` endpoint so the frontend can send a full multi-turn `messages` array instead of a single `prompt`. The single-turn path keeps working unchanged. No new endpoint, no new SSE protocol.
