...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload video to multiple social-media."
    )
    parser.add_argument(
        "platform",
        metavar="platform",
        choices=get_supported_social_media(),
        help="Choose social-media platform: douyin tencent tiktok kuaishou",
    )

    parser.add_argument(
        "account_name", type=str, help="Account name for the platform: xiaoA"
    )
    subparsers = parser.add_subparsers(
        dest="action", metavar="action", help="Choose action", required=True
    )
    ...
    for action in actions:
        ...
        if action == "login":
            ...
            action_parser.add_argument(
                ...
                type=int,
                choices=[0, 1],
                help="0 for immediate, 1 for scheduled",
                default=0,
            )
            action_parser.add_argument(
                "-t", "--schedule", help="Schedule UTC time in %Y-%m-%d %H:%M format"
            )
    ...
    if args.action == "upload":
        if not exists(args.video_file):
            raise FileNotFoundError(
                f"Could not find the video file at {args['video_file']}"
            )
    ...
    account_file = Path(
        BASE_DIR / "cookies" / f"{args.platform}_{args.account_name}.json"
    )
    ...
    if args.action == "login":
        print(
            f"Logging in with account {args.account_name} on platform {args.platform}"
        )
        ...
        if args.platform == SOCIAL_MEDIA_DOUYIN:
            ...
            app = TencentVideo(
                title, video_file, tags, publish_date, account_file, category
            )
        elif args.platform == SOCIAL_MEDIA_KUAISHOU:
