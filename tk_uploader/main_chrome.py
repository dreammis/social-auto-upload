...
class TiktokVideo(object):
    ...
    async def set_schedule_time(self, page, publish_date):
        ...
        if await page.frame_locator(Tk_Locator.tk_iframe).locator('div.TUXButton-content >> text=Allow').count():
            await page.frame_locator(Tk_Locator.tk_iframe).locator('div.TUXButton-content >> text=Allow').click()
        ...
        calendar_month = await page.frame_locator(Tk_Locator.tk_iframe).locator(
    ...
    async def upload_thumbnails(self, page):
        await page.frame_locator(Tk_Locator.tk_iframe).locator(".cover-selector-image-container").click()
        await page.frame_locator(Tk_Locator.tk_iframe).locator(".cover-edit-container >> text=Upload cover").click()
        async with page.expect_file_chooser() as fc_info:
            await page.frame_locator(Tk_Locator.tk_iframe).locator(".upload-image-upload-area").click()
            file_chooser = await fc_info.value
            await file_chooser.set_files(self.thumbnail_path)
        await page.frame_locator(Tk_Locator.tk_iframe).locator('div.cover-edit-panel:not(.hide-panel)').get_by_role(
    ...
    async def click_publish(self, page):
        ...
        while True:
            try:
                publish_button = page.frame_locator(Tk_Locator.tk_iframe).locator('div.button-group button').nth(0)
    ...
    async def detact_upload_status(self, page):
        while True:
            try:
                ...
                if await page.frame_locator(Tk_Locator.tk_iframe).locator(
